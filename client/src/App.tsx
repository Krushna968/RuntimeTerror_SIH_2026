...
export function App() {
  ...
  return (
    <div className="relative flex flex-col min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/40 to-cyan-50/40 text-slate-900 overflow-x-hidden">
      {/* Top Header */}
      <Header
        activeTab={activeTab}
      ...
      <main className="relative z-10 flex-1 p-3 lg:p-6 max-w-[1920px] w-full mx-auto space-y-5">
        ...
        {activeTab === 'agent-lab' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            <div className="lg:col-span-8 space-y-4">
              <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-md space-y-5">
                <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                  <div className="flex items-center space-x-3">
                    <div className="p-2.5 rounded-2xl bg-blue-50 text-blue-700 border border-blue-100 shadow-xs">
                ...
                {latestResponse && (
                  <div className="space-y-3.5 pt-2">
                    {latestResponse.evidence_and_provenance.execution_trace.map((step, idx) => (
                      <div key={idx} className="bg-slate-50 p-4 rounded-2xl border border-slate-200 hover:border-blue-300 hover:bg-white hover:shadow-md transition-all space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-extrabold text-blue-800 flex items-center space-x-2">
                            <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold">
                              {idx + 1}
                            </span>
                            <span>{step.agent}</span>
                          </span>
                          <span className="text-xs font-mono font-bold text-slate-600 bg-white px-2.5 py-0.5 rounded-md border border-slate-200 shadow-2xs">
                            {step.duration_ms} ms
                          </span>
                        </div>
                        <p className="text-xs text-slate-800 font-medium pl-8">{step.thought}</p>
                        <div className="ml-8 p-2.5 rounded-xl bg-white border border-slate-200 text-xs font-mono text-emerald-800 font-bold shadow-2xs">
                          ➔ {step.output_summary}
                        </div>
                      </div>
                    ))}
        ...
        {activeTab === 'bulletin' && (
          <div className="max-w-6xl mx-auto space-y-6">
            {/* Top Action Header */}
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  <span className="px-2.5 py-0.5 rounded-md text-[10px] font-black bg-orange-100 text-orange-700 border border-orange-200 uppercase tracking-widest">
              ...
              <button
                ...
                className="flex items-center justify-center space-x-2 px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-black text-xs shadow-md shadow-blue-500/20 transition-all active:scale-95 cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {/* Card 2: Safety Score */}
              <div className="bg-white p-5 rounded-2xl border border-blue-200 space-y-2 shadow-xs">
              ...
              <div className="bg-white p-5 rounded-2xl border border-amber-200 space-y-2 shadow-xs">
                <div className="flex items-center justify-between text-xs font-bold text-amber-700">
                  <span>PFZ Hotspots Detected</span>
                  <Fish className="w-4 h-4" />
                </div>
                <div className="text-2xl font-black font-mono text-slate-900">
                  {latestResponse?.official_bulletin.recommended_pfz_count || 15} <span className="text-xs text-amber-700 font-bold">Fronts</span>
                </div>
                <div className="text-[11px] text-slate-600">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-4 shadow-md">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                ...
                <span className="text-xs font-mono text-blue-700 font-bold bg-blue-50 px-3 py-1 rounded-full border border-blue-100">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-800">
                    {(latestResponse?.all_pfz_hotspots || pfzHotspots).map((pfz, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        <td className="p-3 font-bold text-blue-900 flex items-center space-x-2">
                        ...
                        <td className="p-3 font-black text-amber-700">{pfz.confidence_score_percent}%</td>
                        <td className="p-3 text-right">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold border border-blue-200 text-[11px] transition-all cursor-pointer"
                          >
                            View on Map ➔
                          </button>
                        </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-5 rounded-2xl border border-red-200 bg-red-50 space-y-2">
      ...
      {/* Emergency SOS Modal */}
      {isSOSModalOpen && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white p-6 md:p-8 rounded-3xl border border-red-200 shadow-2xl space-y-4 text-center">
            <div className="w-16 h-16 rounded-full bg-red-100 border-2 border-red-500 flex items-center justify-center mx-auto text-red-600 animate-pulse">
              <AlertOctagon className="w-9 h-9" />
            </div>

            <h2 className="text-xl font-black text-slate-900">EMERGENCY DISTRESS SOS ACTIVATED</h2>
            <p className="text-xs text-slate-600 font-medium">
              Broadcasting geo-tagged distress packet to Indian Coast Guard Maritime Rescue Co-ordination Centre (MRCC).
            </p>

            <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-xs font-mono text-slate-800 space-y-1.5 text-left">
              <div>Vessel ID: <strong className="text-slate-900">IND-KL-04-M (Kochi)</strong></div>
              <div>GPS Coordinates: <strong className="text-slate-900">9.94°N, 76.25°E</strong></div>
              <div>Distress Frequency: <strong className="text-emerald-700">VHF Channel 16 (156.8 MHz)</strong></div>
            </div>

            <div className="flex items-center space-x-3 pt-2">
              <a
                href="tel:1554"
                className="flex-1 py-3 rounded-2xl bg-red-600 hover:bg-red-700 text-white font-black text-xs flex items-center justify-center space-x-2 shadow-md shadow-red-600/30 transition-all cursor-pointer"
              >
                <PhoneCall className="w-4 h-4" />
